...
def get_list_of_university_towns():
    ...
    # Strip the white space at the beginning and end of each line e.g.'  a' -> 'a'
...
def get_recession_start():
    ...
    df =pd.read_excel('gdplev.xls',skiprows = 220, parse_cols = 'E,G',header=None )
    df.columns = ['Quarter', 'GDP']
     
    for i in range(4,len(df)):
        if (df.loc[i-4, 'GDP'] > df.loc[i-3, 'GDP'])                 and (df.loc[i-3, 'GDP'] > df.loc[i-2, 'GDP'])                 and (df.loc[i-2, 'GDP'] < df.loc[i-1, 'GDP'])                 and (df.loc[i-1, 'GDP'] < df.loc[i, 'GDP']):
            recession_base_idx = i-4
            
    result = df.loc[recession_base_idx, 'Quarter']
    j = recession_base_idx
    while True:
        if (df.loc[j, 'GDP'] < df.loc[j-1, 'GDP']):
            #print("Not first decline",j , df.loc[j, 'GDP'])
            j -= 1
            
        else:
            #print("Yes it is", j+1,  df.loc[j+1, 'GDP'])
            result = df.loc[j+1, 'Quarter']
            break
            
    #print(df.iloc[recession_base_idx-10:recession_base_idx+10])
    return result
get_recession_start()
...
def convert_housing_data_to_quarters():
    ...
    cols_to_keep = ["State","RegionName", 'RegionID']
...
def run_ttest():
    ...
    #print(univ_town_price_ratio.mean(), non_univ_town_price_ratio.mean())
