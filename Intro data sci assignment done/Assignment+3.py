...
def answer_one():
    ...
    new_name = {"Korea, Rep.": "South Korea", 
                    "Iran, Islamic Rep.": "Iran",
                    "Hong Kong SAR, China": "Hong Kong"}
    GDP['Country Name'].replace(to_replace=new_name, inplace=True)
    last10years = np.arange(2006, 2016).astype(str)
    GDP = GDP[np.append(['Country Name'],last10years)]
    GDP.reset_index()
    ...
    merge1 = pd.merge(energy, GDP, how='outer', left_index=True, right_index=True)
    result = pd.merge(ScimEn, merge1, how='outer', left_index=True, right_index=True)
...
def answer_four():
    ...
    last10years = np.arange(2006, 2016).astype(str)
    Top15['avgGDP'] = Top15[last10years].mean(axis = 1)
    Top15['GDP changed'] = Top15['2015'] - Top15['2006']
    Top15 = Top15.sort_values( ['avgGDP'], ascending=False )
    
    return Top15.iloc[6-1].loc['GDP changed']
answer_four()
...
def answer_six():
    ...
    import pandas as pd
    Top15 = answer_one()
    Top = Top15['% Renewable'].argmax()
    Max = Top15['% Renewable'].max()
    MaX_other = Top15.loc[Top,'% Renewable']
    return (Top,Max)
answer_six()
...
def answer_seven():
    ...
    TopMax = Top15['Ratio of Citations'].max()
    NameMax = Top15['Ratio of Citations'].argmax()
    return NameMax, Top15.loc[NameMax,'Ratio of Citations']
answer_seven()
...
def answer_eight():
    Top15 = answer_one()
    Top15['Pops_Estmate'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    Top15 = Top15.sort_values(['Pops_Estmate'], ascending = False)
...
def answer_nine():
    Top15 = answer_one()
    Top15['Pops_Estmate'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    Top15['Es_num_citPERper'] = Top15['Citable documents'] / Top15['Pops_Estmate']
    Top15Corr = Top15.corr()
    return Top15Corr.loc['Es_num_citPERper', 'Energy Supply per Capita']
answer_nine()
...
def answer_eleven():
    ...
    Top15['PopEst'] = Top15['Energy Supply'] / Top15['Energy Supply per Capita']
    #ans = Top15.copy()
    ans = Top15[['Continent', 'PopEst']]
    ans = Top15.groupby('Continent')['PopEst'].agg({'size': np.size,'sum': np.sum,'mean': np.mean,'std': np.std})
    idx = pd.IndexSlice
    return ans
answer_eleven()
...
def answer_twelve():
    ...
    ans = Top15.groupby(['Continent','% Renewable'])['Country'].count()
